...
template<class T> bool HashTable<T>::insert(int key, T value, int& collisions)
{
    ...
    while (isTrue)
    {
        // THIS IS WHAT NEEDS WORK. MAKING SURE THAT THE HASH FUNCTION CAN HIT EVERY SPOT IN HASH TABLE ACCURATELY
        // An offset can be negative... offset just cant be greater than the hash
...
template<class T> bool HashTable<T>::find(int key, T& value)
{
    ...
    // Check if there is a record in that location (by checking if the record is initialized) AND THERE IS NO GRAVE
    ...
    else if (!hashTable[hashTwo(key)].initTrue() && hashTable[hashTwo(key)].getGrave() == 1) // Case 3
    {
        // Probe until a blank space with no grave or until key is found
        bool isTrue = true;
        int offset = 0;
        int i = 0;
        while (isTrue)
