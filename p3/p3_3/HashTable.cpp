...
template<class T> bool HashTable<T>::insert(int key, T value, int& collisions)
{
    ...
    // THIS IS THE CODE FOR INSERTING WITH HASHONE, LINEARPROBE
    ...
    while (isTrue)
    {
        ...
        while (hashOne(key) + offset >= MAXHASH)
        {
            offset -= MAXHASH;
        }

        // If offset landed on a blank space
        if (!hashTable[hashOne(key) + offset].initTrue())
        {
...
{
    // THIS IS THE CODE FOR HASHONE, LINEAR PROBE
    ...
    if (hashTable[hashOne(key)].initTrue())
    {
        ...
        hashTable[hashOne(key)].setInit(false);
...
template<class T> bool HashTable<T>::find(int key, T& value)
{
    ...
    // THIS CODE IS FOR HASHONE, LINEAR PROBE
    // Check if there is a record in that location (by checking if the record is initialized)
    if (hashTable[hashOne(key)].initTrue() && hashTable[hashOne(key)].getGrave() == -1)
    {
        ...
        if (hashTable[hashOne(key)].getKey() == key)
        {
        ...
        else // Use probing to see if the record was placed elsewhere
        {
            ...
            while (isTrue)
            {
                ...
                if (!hashTable[hashOne(key) + offset].initTrue()) // If offset landed on a blank space
                {
                ...
                else // Offset didn't land on a blank space
                {
                    ...
                    if (hashTable[hashOne(key) + offset].getKey() == key)
                    {
                        value = hashTable[hashOne(key) + offset].getValue();
                        return true;
                    }
                    else
                    {
    ...
    else if (!hashTable[hashOne(key)].initTrue() && hashTable[hashOne(key)].getGrave() == 1)
    {
        bool isTrue = true;
        int offset = 0;
        int i = 0;
...
template<class T> int HashTable<T>::doubleHashProbe(int key, int attempts)
{
    ...
    int offset = hashThree(key) + attempts;

    if (offset != 0)
    {
        // Make sure that the offset is relatively prime
        while (MAXHASH % (hashOne(key) + offset) == 0)
        {
